...
if (!paymentMethod) {
setError("Elige el método de pago antes de confirmar la venta.");
return;
}
...
<label className="block text-xs uppercase tracking-wide text-muted-700 font-mono mb-1">
Método de Pago {!paymentMethod && <span className="text-coral-500">(obligatorio)</span>}
</label>
...
<button onClick={handleFinalize} disabled={saving || !paymentMethod} className="btn-primary flex-1 py-3 disabled:opacity-50">
{saving ? "Guardando..." : !paymentMethod ? "Elige método de pago" : "Confirmar Venta"}
</button>
</div>
</section>
</div>
